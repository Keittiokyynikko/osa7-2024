...
const BlogPage = ({handleEdit}) => {
    ...
    return(
        ...
        <a className="mb-4" href={blog.url}>{blog.url}</a>
        <p className="mt-3 mb-1 fs-4">{blog.likes} likes</p>
        <p><button onClick={() => handleEdit(blog)}>Like</button></p>
        ...
        <ul className="p-0 mt-3">
            {blog.comments.map(blog => (
                <li style={listStyle} className="text-end text-decoration-none mb-2" key={blog.id}>{blog.content}</li>
            ))}
        </ul>
        </div>
        </div>
